...
let firstNamesAllCaps = runners.map((value) =>  value.first_name.toUpperCase());
console.log(firstNamesAllCaps);

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue. We need a filtered version of the runners array, containing only those runners with large sized shirts so they can choose a different size. This will be an array of objects.
let runnersLargeSizeShirt = runners.filter((largeShirt) => largeShirt.shirt_size === "L");
...
// Problem 1: just who donaite more then $100 get invetation
let donateOver = runners.filter(function(value){
  return value.donation >= 100;
});
console.log(donateOver);


// Problem 2: first names into alphabetical order

let alphabetFirstName = [];
let alphabetFirstNameSorted = [];
runners.forEach(function(name){
alphabetFirstName.push(name.first_name);
})
alphabetFirstNameSorted = alphabetFirstName.sort();
console.log(alphabetFirstNameSorted);

// Problem 3: add all donation over 100
let totalOver = donateOver.reduce(function(accumulator,currentValue){
  return accumulator+currentValue.donation;
},0);
